...
resource "aws_api_gateway_resource" "cacheProxy" {
  rest_api_id = aws_api_gateway_rest_api.cacheApi.id
  parent_id   = aws_api_gateway_rest_api.cacheApi.root_resource_id
  path_part   = "{proxy+}"
}

resource "aws_api_gateway_method" "cacheProxy" {
  rest_api_id   = aws_api_gateway_rest_api.cacheApi.id
  resource_id   = aws_api_gateway_resource.cacheProxy.id
  http_method   = "ANY"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "cacheLambda" {
  rest_api_id = aws_api_gateway_rest_api.cacheApi.id
  resource_id = aws_api_gateway_method.cacheProxy.resource_id
  http_method = aws_api_gateway_method.cacheProxy.http_method

  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.cacheFunction.invoke_arn
}

resource "aws_api_gateway_method" "cacheProxyRoot" {
  rest_api_id   = aws_api_gateway_rest_api.cacheApi.id
  resource_id   = aws_api_gateway_rest_api.cacheApi.root_resource_id
  http_method   = "ANY"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "cacheLambdaRoot" {
  rest_api_id = aws_api_gateway_rest_api.cacheApi.id
  resource_id = aws_api_gateway_method.cacheProxyRoot.resource_id
  http_method = aws_api_gateway_method.cacheProxyRoot.http_method
...
resource "aws_api_gateway_deployment" "cacheDeployment" {
  depends_on = [
    aws_api_gateway_integration.cacheLambda,
    aws_api_gateway_integration.cacheLambdaRoot,
  ]

  rest_api_id = aws_api_gateway_rest_api.cacheApi.id
  stage_name  = "cacheStage"
}
